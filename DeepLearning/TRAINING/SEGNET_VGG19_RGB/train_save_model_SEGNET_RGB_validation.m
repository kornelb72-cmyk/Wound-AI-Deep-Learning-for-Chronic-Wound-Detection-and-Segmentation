...
for foldNum=1:numFolds
    ...
    if isempty(classificationLayerName)
        error('Could not find a pixel classification layer in the network graph.');
    end

    pxLayer=pixelClassificationLayer('Classes',classNames,'Name',classificationLayerName);
    ...
    try
        ...
        if exist('net','var')
            foldPartialModelName=fullfile(modelsRoot,sprintf('segnet_vgg19_validation_fold_%d_partial.mat',foldNum));
