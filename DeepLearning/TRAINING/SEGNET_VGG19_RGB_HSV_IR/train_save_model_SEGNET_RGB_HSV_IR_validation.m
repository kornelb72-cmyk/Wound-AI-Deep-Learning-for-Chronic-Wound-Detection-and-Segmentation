...
for foldNum=1:numFolds
    ...
    lgraph=segnetLayers(inputSize,numel(classNames),'vgg19');

    layerNames=string({lgraph.Layers.Name});
    classificationLayerName=layerNames(contains(layerNames,'pixel','IgnoreCase',true));
    if isempty(classificationLayerName)
        error('Pixel classification layer not found in network graph.');
    end

    pxLayer=pixelClassificationLayer('Classes',classNames,'Name','pixelOutput');
    lgraph=replaceLayer(lgraph,classificationLayerName,pxLayer);
    ...
    try
        ...
        if exist('net','var')
            foldPartialModelName=fullfile(modelsRoot,sprintf('segnet_vgg19_validation_rgbhsvir_fold_%d_partial.mat',foldNum));
...
function [imdsResized,pxdsResized]=prepareFoldData(foldNames,inputSize,classNames,labelIDs,rootFolder)
    ...
    for i=1:numel(foldNames)
        ...
        for j=1:numel(caseDirs)
            ...
            imageFiles=dir(fullfile(casePath,'*_obraz.png'));
    ...
    [imagesSynced,labelsSynced]=synchronizeDatasets(images,labels,'_obraz','_gt');
