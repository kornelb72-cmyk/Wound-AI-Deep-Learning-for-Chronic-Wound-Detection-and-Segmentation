...
for foldNum=1:numFolds
    ...
    try
        ...
        if exist('net','var')
            foldPartialModelName=fullfile(modelsRoot,sprintf('unet_dice_loss_rgbhsvir_fold_%d_partial.mat',foldNum));
...
function [imdsResized,pxdsResized]=prepareFoldData(foldNames,inputSize,classNames,labelIDs,rootFolder)
    ...
    resizedMasks=cellfun(@(x)imresize(imread(x),inputSize(1:2),'nearest'),labelsSynced,'UniformOutput',false);
