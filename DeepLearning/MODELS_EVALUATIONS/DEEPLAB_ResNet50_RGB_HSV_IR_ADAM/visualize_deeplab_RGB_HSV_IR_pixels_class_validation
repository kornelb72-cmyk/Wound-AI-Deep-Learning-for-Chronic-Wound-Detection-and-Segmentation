...
deepRoot = 'D:/DeepLearning'; %% OWN PATH

evalRoot = fullfile(deepRoot,'MODELS_EVALUATIONS','DEEPLAB_ResNet50_RGB_HSV_IR_ADAM');
if ~isfolder(evalRoot), mkdir(evalRoot); end

modelsRoot = fullfile(deepRoot,'MODELS','DEEPLAB_ResNet50_RGB_HSV_IR_ADAM');
...
mainResultsRoot = fullfile(evalRoot,'Segmentation_Results_DEEPLABV3PLUS_RGB_HSV_IR_VALIDATION');
...
fprintf('Creating test data for Fold %d...\n', foldNum);
testFoldName = sprintf('Fold%d', foldNum);
testFoldPath = fullfile(deepRoot, testFoldName);
...
error('The output folder does not exist: %s', resultsFolder);
end
...
error('No matching images and masks found in the folder: %s', testFoldPath);
end
...
excelFileName = fullfile(statsRoot,'SegmentationMetrics_deeplabv3plus_resnet50_RGB_HSV_IR_ADAM.xlsx');
