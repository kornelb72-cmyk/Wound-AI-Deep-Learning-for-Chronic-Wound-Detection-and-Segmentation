...
evalRoot = fullfile(rootFolder,'MODELS_EVALUATIONS','DEEPLAB_ResNet50_RGB_ADAM');
if ~isfolder(evalRoot), mkdir(evalRoot); end

modelsRoot = fullfile(rootFolder,'MODELS','DEEPLAB_ResNet50_RGB_ADAM');
...
mainResultsRoot = fullfile(evalRoot,'Segmentation_Results_DEEPLABV3PLUS_RGB_VALIDATION');
...
foldModelName = fullfile(modelsRoot,sprintf('deeplabv3plus_resnet50_validation_fold_%d.mat', foldNum));
if isfile(foldModelName)
load(foldModelName,'net','info');
fprintf('Model for Fold %d loaded from %s\n', foldNum, foldModelName);
...
error('Result folder does not exist: %s', resultsFolder);
end
...
fprintf('Tissue classification summary for Fold %d saved to Excel: %s\n', foldNum, excelFile);
end
...
fprintf('Loading and preprocessing test data from: %s\n', testFoldPath);
imageFiles = dir(fullfile(testFoldPath, '**', '*_rgb.*'));
labelFiles = dir(fullfile(testFoldPath, '**', '*_gt.*'));
if isempty(imageFiles) || isempty(labelFiles)
error('No images or masks found in folder: %s', testFoldPath);
end
...
error('No matching image–mask pairs found in folder: %s', testFoldPath);
end
...
otherIdx = mask ~= bgVal & mask ~= woundVal;
...
error('Image %s has size [%d, %d], expected [%d, %d].', ...
imds.Files{i}, size(img, 1), size(img, 2), inputSize(1), inputSize(2));
end
end
for i = 1:numel(pxds.Files)
mask = imread(pxds.Files{i});
if ~isequal(size(mask, 1:2), inputSize(1:2))
error('Mask %s has size [%d, %d], expected [%d, %d].', ...
pxds.Files{i}, size(mask, 1), size(mask, 2), inputSize(1), inputSize(2));
end
if size(mask, 3) ~= 1
error('Mask %s has an invalid number of channels: %d. Expected 1.', ...
pxds.Files{i}, size(mask, 3));
end
end
end
...
fprintf('Running semantic segmentation for Fold %d...\n', foldNum);
...
fprintf('Saving predicted wound masks to folder: %s\n', outputDir);
...
fprintf('Generating visualization for all test images (Fold %d)...\n', foldNum);
...
fprintf('Visualization results saved in folder: %s\n', outputDir);
end
...
if nTrue ~= nPaths
warning('Fold %d: number of GT masks (%d) differs from number of image paths (%d).', foldNum, nTrue, nPaths);
end
...
excelFileName = fullfile(statsRoot, 'SegmentationMetrics_deeplabv3plus_resnet50_RGB_ADAM.xlsx');
...
existingMetrics = readtable(excelFileName, 'Sheet', metricsSheetName, ...
'VariableNamingRule', 'preserve');
metricsTable = [existingMetrics; metricsTable];
end
if ismember(totalsSheetName, sheets)
existingTotals = readtable(excelFileName, 'Sheet', totalsSheetName, ...
'VariableNamingRule', 'preserve');
totalsTable = [existingTotals; totalsTable];
end
if ismember(perImageSheetName, sheets)
existingPerImg = readtable(excelFileName, 'Sheet', perImageSheetName, ...
'VariableNamingRule', 'preserve');
neededVars = {'Fold','ImagePath','BackgroundPixels','WoundPixels', ...
'BackgroundPercent','WoundPercent','TotalPixels', ...
'TP','TN','FP','FN'};
if all(ismember(neededVars, existingPerImg.Properties.VariableNames)) ...
&& width(existingPerImg) == numel(neededVars)
perImageTable = [existingPerImg; perImageTable];
else
warning('Existing sheet %s has a different structure and will be overwritten.', perImageSheetName);
end
end
if ismember(pixelCountSheet, sheets)
existingPixelCounts = readtable(excelFileName, 'Sheet', pixelCountSheet, ...
'VariableNamingRule', 'preserve');
...
fprintf('Segmentation metrics saved to: %s\n', excelFileName);
...
warning('No matching image–mask pairs found.');
end
end
