...
evalRoot=fullfile(rootFolder,'MODELS_EVALUATIONS','DEEPLAB_ResNet50_RGB_SGDM');
if ~isfolder(evalRoot),mkdir(evalRoot);end
modelsRoot=fullfile(rootFolder,'MODELS','DEEPLAB_ResNet50_RGB_SGDM');
if ~isfolder(modelsRoot),error('Models folder does not exist: %s',modelsRoot);end
...
mainResultsRoot=fullfile(evalRoot,'Segmentation_Results_DEEPLABV3PLUS_RGB_VALIDATION_SGDM');
...
metrics=evaluateModel(net,pxdsTestResized,imdsTestResized,foldNum,inputSize,rootFolder,classNames,origImagePaths,visRoot,maskRoot,statsRoot);
...
fprintf('Tissue type classification in predicted mask for Fold %d...\n',foldNum);
healthyClasses=categorical("background");
diseasedClasses=categorical("wound");
resultsTable=table('Size',[0,3],...
'VariableTypes',{'double','double','double'},...
'VariableNames',{'Image','Healthy tissues [%]','Pathological changes [%]'});
...
fprintf('Summary of the results for Fold %d has been saved to Excel file: %s\n',foldNum,excelFile);
end
...
function metrics=evaluateModel(net,pxdsTestResized,imdsTestResized,foldNum,inputSize,rootFolder,classNames,origImagePaths,visRoot,maskRoot,statsRoot)
...
excelFileName=fullfile(statsRoot,'SegmentationMetrics_DEEPLAB_ResNet50_RGB_SGDM.xlsx');
