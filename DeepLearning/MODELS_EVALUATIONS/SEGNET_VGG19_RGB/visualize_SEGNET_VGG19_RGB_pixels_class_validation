...
evalRoot   = fullfile(rootFolder,'MODELS_EVALUATIONS','SEGNET_VGG19_RGB');
...
mainResultsRoot = fullfile(evalRoot,'Segmentation_Results_SEGNET_RGB_VALIDATION');
...
function metrics = calculateMetrics(pxdsTrue, pxdsPred, foldNum, rootFolder, origImagePaths)
...
pixelCountsTable = table(foldColumn, classNames', pixelPercentages','VariableNames', {'NumFold','Class','Number of pixels [%]'});
...
excelFileName  = fullfile(rootFolder,'SegmentationMetrics_Segnet_VGG19_RGB_validation.xlsx');
...
'IoU','MCC','AUC'});
totalsTable = table(foldNum, totalTP, totalTN, totalFP, totalFN,'VariableNames', {'Fold','TotalTP','TotalTN','TotalFP','TotalFN'});
