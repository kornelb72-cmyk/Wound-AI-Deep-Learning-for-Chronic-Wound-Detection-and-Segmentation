...
evalRoot=fullfile(rootFolder,'MODELS_EVALUATIONS','UNET_DICE_LOSS_RGB');
...
segResultsRoot=fullfile(evalRoot,'Segmentation_Results_UNET_RGB_DICE_LOSS');
...
testImages={};
testLabels={};
caseDirs=dir(fullfile(testFoldPath,'case_*'));
caseDirs=caseDirs([caseDirs.isdir]);

for j=1:length(caseDirs)
casePath=fullfile(testFoldPath,caseDirs(j).name);
imageFiles=dir(fullfile(casePath,'*_rgb.*'));
labelFiles=dir(fullfile(casePath,'*_gt.*'));
...
testImages=[testImages;imagePaths(:)];
testLabels=[testLabels;labelPaths(:)];
end
end

fprintf('Synchronizing test set...\n');
[testImagesSynced,testLabelsSynced]=synchronizeDatasets(testImages,testLabels,'_rgb','_gt');
fprintf('Test set after synchronization: %d images and %d masks.\n',numel(testImagesSynced),numel(testLabelsSynced));
...
excelFile=fullfile(resultsFolder,sprintf('Fold_%d_Results_UNET_RGB.xlsx',foldNum));
...
[~,imgIdx,labelIdx]=intersect(imageNamesClean,labelNamesClean,'stable');
...
fprintf('Starting semantic segmentation (UNET RGB)...\n');
...
excelFileName=fullfile(statsRoot,'SegmentationMetrics_UNET_DICE_LOSS_RGB.xlsx');
