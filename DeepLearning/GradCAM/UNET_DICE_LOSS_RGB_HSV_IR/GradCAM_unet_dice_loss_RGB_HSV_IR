...
mainGradcamDir=fullfile(rootFolder,'GradCAM','UNET_DICE_LOSS_RGB_HSV_IR');
...
for foldNum=1:numFolds
    ...
    fprintf('Found %d test images in %s.\n',numTestImages,testFoldName);
    ...
    foldDir=fullfile(mainGradcamDir,sprintf('Fold%d',foldNum));
    if ~isfolder(foldDir)
        mkdir(foldDir);
    end
    ...
    for i=1:thisMaxImagesToProcess
        ...
        imwrite(camNorm,fullfile(foldDir,[baseName '_cam_gray.png']));
        imwrite(camRGB,fullfile(foldDir,[baseName '_cam_color.png']));
        imwrite(overlayGradCAM,fullfile(foldDir,[baseName '_overlay_gradcam.png']));
        imwrite(predOverlay,fullfile(foldDir,[baseName '_overlay_predMask.png']));
        imwrite(overlayGlobalCAM,fullfile(foldDir,[baseName '_overlay_globalCAM.png']));
        imwrite(overlayMaskedCAM,fullfile(foldDir,[baseName '_overlay_maskedCAM.png']));

        saveas(hFig,fullfile(foldDir,[baseName '_summary.png']));
        close(hFig);

        save(fullfile(foldDir,[baseName '_cam.mat']),'cam');
    end

    fprintf('\nFinished Grad-CAM for Fold %d (processed %d images).\n',foldNum,thisMaxImagesToProcess);
