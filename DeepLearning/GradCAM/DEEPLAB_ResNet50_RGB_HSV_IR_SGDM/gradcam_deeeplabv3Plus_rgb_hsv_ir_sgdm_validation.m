...
mainGradcamDir=fullfile(rootFolder,'GradCAM','DEEPLAB_ResNet50_RGB_HSV_IR_SGDM');
if ~isfolder(mainGradcamDir)
    mkdir(mainGradcamDir);
end
fprintf('Main results folder (MIXED+SGDM): %s\n', mainGradcamDir);

for foldNum=1:numFolds

    fprintf('\n=============================================\n');
    fprintf('=== DeepLabv3+ RGB_HSV_IR+SGDM – FOLD %d / %d ===\n', foldNum, numFolds);
    fprintf('=============================================\n');
    ...
    fprintf('\nFinished Fold %d (MIXED+SGDM).\n',foldNum);
end

fprintf('\nFinished generating Grad-CAM (MIXED+SGDM) for all folds.\n');
...
function [imdsResized,pxdsResized]=loadAndProcessTestData(testFoldPath,classNames,labelIDs,inputSize)
    ...
    tempImageDir=fullfile(tempdir,'ProcessedImages_DeepLab_MIXED_SGDM');
    tempLabelDir=fullfile(tempdir,'ProcessedMasks_DeepLab_MIXED_SGDM');
