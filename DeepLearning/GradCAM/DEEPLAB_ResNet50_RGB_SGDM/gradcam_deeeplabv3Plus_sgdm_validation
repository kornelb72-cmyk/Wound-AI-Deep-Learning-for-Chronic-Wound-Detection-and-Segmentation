...
mainGradcamDir=fullfile(rootFolder,'GradCAM','DEEPLAB_ResNet50_RGB_SGDM');
if ~isfolder(mainGradcamDir)
    mkdir(mainGradcamDir);
end
fprintf('Main results folder (SGDM): %s\n',mainGradcamDir);

for foldNum=1:numFolds

    fprintf('\n=============================================\n');
    fprintf('=== DeepLabv3+ SGDM – FOLD %d / %d ===\n',foldNum,numFolds);
    fprintf('=============================================\n');
    ...
    fprintf('\n>>> Finished Fold %d (SGDM).\n',foldNum);
end

fprintf('\nFinished generating Grad-CAM (SGDM) for all folds.\n');
...
function [imdsResized,pxdsResized]=loadAndProcessTestData(testFoldPath,classNames,labelIDs,inputSize)
    ...
    tempImageDir=fullfile(tempdir,'ProcessedImages_DeepLab_SGDM');
    tempLabelDir=fullfile(tempdir,'ProcessedMasks_DeepLab_SGDM');
