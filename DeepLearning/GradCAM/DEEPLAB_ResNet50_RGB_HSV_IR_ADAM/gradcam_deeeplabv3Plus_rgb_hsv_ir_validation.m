...
modelsRoot=fullfile(rootFolder,'MODELS','DEEPLAB_ResNet50_RGB_HSV_IR_ADAM');

mainGradcamDir=fullfile(rootFolder,'GradCAM','DEEPLAB_ResNet50_RGB_HSV_IR_ADAM');
if ~isfolder(mainGradcamDir)
    mkdir(mainGradcamDir);
end
fprintf('Main results folder (MIXED): %s\n', mainGradcamDir);

for foldNum=1:numFolds

    fprintf('\n=============================================\n');
    fprintf('=== DeepLabv3+ RGB_HSV_IR – FOLD %d / %d ===\n', foldNum, numFolds);
    fprintf('=============================================\n');

    foldModelName=fullfile(modelsRoot,sprintf('deeplabv3plus_resnet50_validation_mixed_fold_%d.mat', foldNum));
    ...
    thisMaxImages=min(maxImagesToProcess,numTestImages);
    ...
    for i=1:thisMaxImages
        fprintf('\n=== Fold %d — Image %d / %d ===\n', foldNum, i, thisMaxImages);
        ...
        [cam,featureLayerName,scoreLayerName]=gradCAM_segmentation(net,I,woundClassIdx);

        fprintf('Used layers: featureLayer = %s, scoreLayer = %s\n',featureLayerName,scoreLayerName);

        woundMaskPred=predictedMask=='wound';

        scores=activations(net,I,'scorer');
        ...
        alpha=0.5;
        overlayGradCAM=alpha*camRGB+(1-alpha)*I_double;
        ...
        title('Global CAM (scorer)','FontWeight','bold');
    ...
    fprintf('\nFinished Fold %d (MIXED).\n', foldNum);
end

fprintf('\nFinished generating Grad-CAM (MIXED) for all folds.\n');
...
function [imdsResized,pxdsResized]=loadAndProcessTestData(testFoldPath,classNames,labelIDs,inputSize)
    ...
    tempImageDir=fullfile(tempdir,'ProcessedImages_DeepLab_MIXED');
    tempLabelDir=fullfile(tempdir,'ProcessedMasks_DeepLab_MIXED');
